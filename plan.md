...
- В работе: **CP0 — Foundation SPA**, **CP1 — Auth и сессия**
- Следующий: **CP2 — Справочники (счета, категории, валюты)**
...
- `frontend/src/app/(auth)/*` и `frontend/src/app/(protected)/*` как клиентские маршруты.
...
- [ ] Настроены группы маршрутов `(auth)` и `(protected)`.
- [ ] Client-side навигация работает без full reload.
- [ ] Реализован route guard для приватных страниц.
- [ ] Есть AppShell с mobile bottom-nav.
...
- [ ] Login/Register формы и валидация.
- [ ] Access/refresh токены, авто-refresh.
- [ ] Logout с полной очисткой сессии.
...
- [ ] CRUD экран для счетов.
- [ ] CRUD экран для категорий.
- [ ] Подключен справочник валют в формах счетов.
